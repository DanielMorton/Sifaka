...
mod tests {
    ...
    fn test_l2_norm() {
        let outer = CsVecI::new(4, vec![0, 2, 3],
                                vec![4.1412075533, 1.72069753, 4.67]);
        let inner = CsVecI::new(5, vec![0, 1, 3],
                                vec![3.5332704, 2.7, 4.7058793]);
        ...
        (&A_FLOAT.col_l2_norm() - &outer).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&A_FLOAT.row_l2_norm() - &inner).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&B_FLOAT.col_l2_norm() - &inner).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&B_FLOAT.row_l2_norm() - &outer).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
    }
    ...
    fn test_normalize() {
        ...
        assert_eq!(B_FLOAT.col_normalize().ind_vec(), in_norm.transpose_view().ind_vec());
        assert_eq!(B_FLOAT.row_normalize().ind_vec(), out_norm.transpose_view().ind_vec());
        assert_eq!(A_FLOAT.col_normalize().ip_vec(), out_norm.ip_vec());
        assert_eq!(A_FLOAT.row_normalize().ip_vec(), in_norm.ip_vec());
        assert_eq!(B_FLOAT.col_normalize().ip_vec(), in_norm.transpose_view().ip_vec());
        assert_eq!(B_FLOAT.row_normalize().ip_vec(), out_norm.transpose_view().ip_vec());
        (&A_FLOAT.col_normalize() - &out_norm).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&A_FLOAT.row_normalize() - &in_norm).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&B_FLOAT.col_normalize() - &in_norm.transpose_view()).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
        (&B_FLOAT.row_normalize() - &out_norm.transpose_view()).data_vec().iter().for_each(|x| assert_approx_eq!(x, 0.0));
    }
}
